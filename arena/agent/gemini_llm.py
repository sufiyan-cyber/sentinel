"""The target agent: Google Gemini API via official OpenAI-compatible endpoint.

Google Gemini exposes an OpenAI-compatible endpoint at
`https://generativelanguage.googleapis.com/v1beta/openai/`, allowing AgentDojo's
`LocalLLM` to drive it with tool/function calling support.
...
class GeminiLLM(LocalLLM):
    """`LocalLLM` pointed at Google Gemini API without incompatible OpenAI seed parameter."""
    ...
    def __init__(
        ...
        resolved_base = (base_url or config.GEMINI_BASE_URL).rstrip("/")
        resolved_key = (api_key or config.GEMINI_API_KEY).strip()
